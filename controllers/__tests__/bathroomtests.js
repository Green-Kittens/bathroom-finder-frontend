...
describe("getAllBathrooms", () => {
  it("should return all bathrooms", async () => {
    ...
    const bathrooms = [{ id: "1", name: "bathroom1", location: {coordinates:[1], type: ""}, category:"",tags:[""], operations:"",date:"", ratingAVG:4, favorites:2,reports:0}];
...
describe("getBathroomProfile", () => {
  it("should return a bathroom profile", async () => {
    const mock = new MockAdapter(axios);
    const bathroomProfile = { id: "1", name: "bathroom1", location: {coordinates:[1], type: ""}, category:"",tags:[""], operations:"",date:"", ratingAVG:4, favorites:2,reports:0};
    mock.onGet(`http://${host}:${port}/facilities/1`).reply(200, bathroomProfile);
  ...
  it("should handle errors", async () => {
    ...
    await expect(getBathroomProfile("1")).rejects.toThrow("Request failed with status code 500");
  });
});
